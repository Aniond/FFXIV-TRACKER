...
function foodBuff(result, foodMap) {
  return null; // GSM recipes generally do not use food stats in Teamcraft
}

function isGoldsmithRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail GSM recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  const dishes = recipes.filter(r =>
    r.job === GSM_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isGoldsmithRecipe(r.result, nameOf(r.result)));
  ...
  const dest = path.join(__dirname, '..', 'goldsmith-recipes.json');
