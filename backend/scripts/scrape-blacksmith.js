...
function foodBuff(result, foodMap) {
  return null; // BSM recipes generally do not use food stats in Teamcraft
}

function isBlacksmithRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail BSM recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  // ---- 1. BLACKSMITH RECIPES -------------------------------------------
  const dishes = recipes.filter(r =>
    r.job === BSM_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isBlacksmithRecipe(r.result, nameOf(r.result)));
  ...
  const dest = path.join(__dirname, '..', 'blacksmith-recipes.json');
