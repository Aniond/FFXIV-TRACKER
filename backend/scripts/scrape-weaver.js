...
function foodBuff(result, foodMap) {
  return null; // WVR recipes generally do not use food stats in Teamcraft
}

function isWeaverRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail WVR recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  const dishes = recipes.filter(r =>
    r.job === WVR_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isWeaverRecipe(r.result, nameOf(r.result)));
  ...
  const dest = path.join(__dirname, '..', 'weaver-recipes.json');
