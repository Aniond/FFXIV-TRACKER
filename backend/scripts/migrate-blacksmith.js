...
// Blacksmith migration — seeds the recipes table from
// blacksmith-recipes.json (produced by scrape-blacksmith.js). Re-runnable: it
// clears and reseeds the BSM slice each time.
//
// Run against prod from a local machine (see reference-railway-ops):
//   railway run sh -c 'DATABASE_URL=$DATABASE_PUBLIC_URL node scripts/migrate-blacksmith.js'
async function migrate() {
  console.log('Running blacksmith migration…');
  ...
  const seed = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'blacksmith-recipes.json'), 'utf8'));

  // Delete all BSM items that are not subcrafts
  await pool.query('DELETE FROM recipes WHERE job = $1 AND is_subcraft = false', ['BSM']);
  
  // We need to handle subcrafts carefully so we don't duplicate them or delete cooking subcrafts.
  // Easiest is to delete BSM subcrafts that don't overlap, or just do ON CONFLICT if we had a unique constraint.
  // Since we don't have a unique constraint on (name), let's just delete ALL BSM rows, and also delete subcrafts
  // that we are going to insert anyway to avoid duplicates.
  // Actually, wait: `recipes` might have multiple of the same subcraft if both BSM and CUL seed it.
  // It's cleaner to just delete BSM job recipes, and also delete subcrafts by name that we are about to insert
  ...
  console.log(`  seeded ${n} recipes (${dishes} blacksmith recipes + ${subs} subcrafts)`);
  console.log('Blacksmith migration complete.');
  await pool.end();
}
