...
const JOB_ABBR = { 8: 'CRP', 9: 'BSM', 10: 'ARM', 11: 'GSM', 12: 'LTW', 13: 'WVR', 14: 'ARM', 15: 'CUL' };
...
function foodBuff(result, foodMap) {
  return null; // ARM recipes generally do not use food stats in Teamcraft
}

function isArmorerRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail ARM recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  // ---- 1. ARMHEMY CONSUMABLES -------------------------------------------
  const dishRecipes = recipes.filter((r) =>
    r.job === ARM_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isArmorerRecipe(r.result, nameOf(r.result)));
  ...
  const dest = path.join(__dirname, '..', 'armorer-recipes.json');
