...
// Alchemy migration — seeds the recipes table from
// alchemy-recipes.json (produced by scrape-alchemy.js). Re-runnable: it
// clears and reseeds the ALC slice each time.
//
// Run against prod from a local machine (see reference-railway-ops):
//   railway run sh -c 'DATABASE_URL=$DATABASE_PUBLIC_URL node scripts/migrate-alchemy.js'
async function migrate() {
  console.log('Running alchemy migration…');
  ...
  const seed = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'alchemy-recipes.json'), 'utf8'));

  // Delete all ALC items that are not subcrafts
  await pool.query('DELETE FROM recipes WHERE job = $1 AND is_subcraft = false', ['ALC']);
  
  // We need to handle subcrafts carefully so we don't duplicate them or delete cooking subcrafts.
  // Easiest is to delete ALC subcrafts that don't overlap, or just do ON CONFLICT if we had a unique constraint.
  // Since we don't have a unique constraint on (name), let's just delete ALL ALC rows, and also delete subcrafts
  // that are part of this seed to avoid duplicates, or just insert them.
  // Actually, wait: `recipes` might have multiple of the same subcraft if both ALC and CUL seed it.
  // It's cleaner to just delete ALC job recipes, and also delete subcrafts by name that we are about to insert
  ...
  console.log(`  seeded ${n} recipes (${dishes} alchemy recipes + ${subs} subcrafts)`);
  console.log('Alchemy migration complete.');
  await pool.end();
}
