 ...
 *      ANY crafting job (ARM, CUL, WVR, …). These carry no food buff but MUST
...
function foodBuff(result, foodMap) {
  return null; // LTW recipes generally do not use food stats in Teamcraft
}

function isLeatherworkerRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail LTW recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  // ---- 1. LEATHERWORKER RECIPES -------------------------------------------
  const dishes = recipes.filter(r =>
    r.job === LTW_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isLeatherworkerRecipe(r.result, nameOf(r.result)));
  const dishIds = new Set(dishes.map((r) => r.result));
  ...
  const dest = path.join(__dirname, '..', 'leatherworker-recipes.json');
