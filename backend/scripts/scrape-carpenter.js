...
function foodBuff(result, foodMap) {
  return null; // CRP recipes generally do not use food stats in Teamcraft
}

function isCarpenterRecipe(id, name) {
  // User requested ALL Endwalker & Dawntrail CRP recipes (including housing, glamour, standard leveling, etc)
  return true;
}
...
async function main() {
  ...
  const dishes = recipes.filter(r =>
    r.job === CRP_JOB && ilvlOf(r.result) >= EW_MIN_ILVL && isCarpenterRecipe(r.result, nameOf(r.result)));
  ...
  const dest = path.join(__dirname, '..', 'carpenter-recipes.json');
